/* Timestamp: Wed Jul 17 22:29:49 PDT 2019 , Code Version: 1563427784989 */
if(!window["v3Lander"]&&navigator.userAgent.indexOf("Opera")==-1&&navigator.userAgent.indexOf("MSIE 6")==-1&&(navigator.userAgent.indexOf("MSIE 7")==-1||navigator.userAgent.indexOf("Trident")!=-1||navigator.userAgent.toLowerCase().indexOf("media center")!=-1)){if(typeof window.v3LanderConfig!="object"){try{if(parent&&parent.v3LanderConfig){window.v3LanderConfig={parent:parent.v3LanderConfig};}else if(parent.opener&&parent.opener.v3LanderConfig){window.v3LanderConfig={opener:parent.opener.v3LanderConfig};}else{window.v3LanderConfig={};}}catch(e){window.v3LanderConfig={};}}
function getParentV3LanderConfig(){return v3LanderConfig.parent;}
function getOpenerV3LanderConfig(){return v3LanderConfig.parent&&v3LanderConfig.parent.opener;}
function getV3LanderConfigProperty(propertyName,defaultValue){defaultValue=defaultValue||null;if(v3LanderConfig[propertyName]){return v3LanderConfig[propertyName];}else if(getParentV3LanderConfig()&&getParentV3LanderConfig()[propertyName]){return getParentV3LanderConfig()[propertyName];}else if(getOpenerV3LanderConfig()&&getOpenerV3LanderConfig()[propertyName]){return getOpenerV3LanderConfig()[propertyName];}else{return defaultValue;}}
window.v3Lander={FRAME_ID:"inqChatStage",codeVersion:'1563427784989',v3Frame:false,domState:"defer-failed",domReady:false,secureProtocol:function(url){return url.replace(/^HTTPS?:/i,"https:");},hostToPath:{},hostedFileURL:getV3LanderConfigProperty("hostedFileURL","/nuance/nuance-chat.html"),isAtHostedFileURL:false,log:function(s){if(typeof console!="undefined"&&console.log){console.log(s);}},FMProxy:{fcns:{},addFcn:function(name,fcnStr){var obj=null;try{obj=eval("({'"+name+"':"+fcnStr+"})");}
...
return obj;}},_getHostedFileURL:function(domain){if(!domain){domain=window.location.host;}
...
url=window.location.protocol+url;return url;},establishHostedFileURL:function(){var hostPaths=getV3LanderConfigProperty("hostToPath",[]);for(var i=0,paths=hostPaths;i<paths.length;i++){if(paths[i].domain&&paths[i].hostedFileDirectory){v3Lander.hostToPath[paths[i].domain.toUpperCase()]=paths[i].hostedFileDirectory+v3Lander.hostedFileURL;}}
...
+'?IFRAME'+(window.v3LanderConfig.iframeSrcParams||""):v3Lander.hostedFileURL;if(iframesrc.indexOf("http")!=0)return;if(typeof(CEAPI_POST_CHAT_SURVEY_DATA)!="undefined"&&CEAPI_POST_CHAT_SURVEY_DATA!=null){iframesrc=doc.location+'?IFRAME&hostedFileURL=true&siteID='+10005809+'&vanityDomain='+v3Lander.secureProtocol(window.v3LanderConfig.vanityDomain||'https://shutterstock.inq.com');}
...
additionalTags();}
v3Lander.fixV3IFrames(div);function additionalTags(){var divs=div.getElementsByTagName("DIV");for(var ix=divs.length-1;ix>-1;--ix){bdy.appendChild(divs[ix]);}}},loadScript:function(name,useMediaServer,useClientDomain){var bdy=document.getElementsByTagName("BODY");if(bdy&&bdy[0]){var srctag=document.createElement("SC"+"RIPT");if(false&&useClientDomain){v3LanderConfig.clientStaticUrl=getV3LanderConfigProperty("clientStaticUrl","");srctag.src=v3Lander.secureProtocol(window.v3LanderConfig.clientStaticUrl)+name;}else if(useMediaServer){srctag.src=v3Lander.secureProtocol(window.v3LanderConfig.vanityDomain||"http://media-lax1.inq.com"||"https://shutterstock.inq.com")+name;}else{srctag.src=v3Lander.secureProtocol(window.v3LanderConfig.vanityDomain||"https://shutterstock.inq.com")+name;}
srctag.type="text/javascript";srctag.charset="utf-8";srctag.language="javascript";bdy[0].appendChild(srctag);}},inqSiteDataFun:null,inqRulesEngineFun:null,mbusToLoad:[],page:null,cgIDs:null,isBupMode:false,isRplMode:false,loadedMbuRuleDataFuns:[],jsonAPIData:null,jsonMbuData:null,requestPage:function(){this.resolvePage();},getPage:function(){return this.page;},getContentGroupIDs:function(){return this.cgIDs;},resolvePage:function(){var requestUrl="/tagserver/launch/resolvePage?siteID=10005809&url="+encodeURIComponent(document.URL)+"&codeVersion="+encodeURIComponent(v3Lander.codeVersion);var overriddenPageId=null;if(window.inqSiteID){overriddenPageId=window.inqSiteID;}else if(window.parent&&window.parent.inqSiteID){overriddenPageId=window.parent.inqSiteID;}
...
v3Lander.testReady();}};v3Lander.establishHostedFileURL();if(window.location.href.indexOf("surveys/launchNativePostChatSurvey")>-1){(v3Lander).isAtHostedFileURL=window.location.href.indexOf("hostedFileURL=true")>-1;}else{(v3Lander).isAtHostedFileURL=window.location.href.indexOf(v3Lander.hostedFileURL)>-1;}
(v3Lander).assignDomain();if(window.location.href.indexOf("?CLEAR")!=-1){var cookies=document.cookie.split(";");for(var i=0;i<cookies.length;i++){var cookie=cookies[i];var eqPos=cookie.indexOf("=");var cookieName=eqPos>-1?cookie.substr(0,eqPos):cookie;document.cookie=cookieName+'=;Path=/;Expires=Thu, 01 Jan 1970 00:00:01 GMT;';}}
else if(window.location.href.indexOf("?BLNK")!=-1){}
else if(window.location.href.indexOf("?CCFRM")!=-1){var host=(window.v3LanderConfig.vanityDomain||"https://shutterstock.inq.com").replace(/^https?:\/\//,"");var url="http://"+window.document.domain+"/tagserver/ccform/getCreditCardForm?host="+host;window.location.replace(url);}
else if(window.location.href.indexOf("?CCTKN=1")!=-1){var s=document.createElement("script");var params=v3Lander.encodeURLIfNotEncoded(window.location.href.replace(/^.*\?/,"?"));var src=(window.v3LanderConfig.vanityDomain||"https://shutterstock.inq.com")+"/tagserver/cctoken/getToken"+params;s.src=src;document.head.appendChild(s);}
else if(window.location.href.indexOf("?CCTKN=2")!=-1){var s=document.createElement("script");var params=v3Lander.encodeURLIfNotEncoded(window.location.href.replace(/^.*\?/,"?"));var src=(window.v3LanderConfig.vanityDomain||"https://shutterstock.inq.com")+"/tagserver/cctokentest/getToken"+params;s.src=src;document.head.appendChild(s);}
...
else if(window.location.href.indexOf("?POST2SERVER")!=-1){var href=window.location.href;if(href.indexOf("&URL")!=-1){var query=window.location.search.replace(/&eq;/g,'=');var qs=query.slice(1)&&query.slice(1).split('&');if(qs){window.v3LanderConfig.clientStaticUrl=decodeURI(qs[1].split("=")[1])}}
