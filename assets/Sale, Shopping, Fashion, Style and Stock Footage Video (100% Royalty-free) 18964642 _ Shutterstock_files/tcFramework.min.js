...
var site=initSiteData();var one2ManyChatrouterDomain=site.oneToManyChatrouterDomain||"api.inq.com";var one2ManyChat=false;var isConversationActive=false;function getConversationActive(){return isConversationActive;}
...
if(visitedPages.length>0){var data={pages:visitedPages,siteID:getSiteID(),url:encodeURIComponent(document.URL),codeVersion:v3Lander.codeVersion};ROM.post(urls.resolvePageURL,data,null,null,function(context){if(context.data&&context.data.pages){this.pages=context.data.pages;}}.bind(this));}};LE12.prototype._resolvePage=function(){var pages=this.pages;var pageURL=win.location.href;for(var i in pages){var re=pages[i].re;if(re===""){continue;}
...
this.absorb(dat);if(setFocus===true||typeof this.lastDat==="undefined"||this.lastDat.result!==dat.result||this.lastDat.launchable!==dat.launchable||!this.getDiv()||!this.getDiv().firstChild){if(this.isExtC2C){this.renderC2CbyClient();}else{this.show(setFocus);}}
...
SVYM.setSurveyShown(0);}catch(e){lmt12("Failure to Launch: "+cf21(e),LOG_LEVELS.INFO);log("Error parsing Chat Data: \n"+e+"\n"+chatData);if(this.earlyPopout){try{this.earlyPopout.close();}catch(e){}}}finally{if(this.chatRequestQ.length>0){this.request();}}};GR34.prototype.reinitializeChatWithNewId=function(){var callbackFcn=function(response){var jsonResponse=typeof response=='string'?JSON.parse(response):response;var responseData=jsonResponse.data;this.chat.cd.id=responseData["chatID"];this.chat.setAgentConfig(null,null,null,null);this.chat.cd.closed=0;this.lastChat.id=""+responseData;log("Chat ID has been regenerated to "+responseData);this.setDisconnectFlag(0);var surveyData=SVYM.asyncSurveyMgr&&SVYM.asyncSurveyMgr.eventData;this.launchChat(this.chat,true);this.save();if(surveyData){SVYM.asyncSurveyMgr.setEventDataForSurvey(surveyData);}}.bind(this);var url=urls.getNewChatIdUrl;var data={"siteID":Inq.getSiteID()};ROM.post(url,data,null,false,callbackFcn,null);};GR34.prototype.resetStateAfterPersistentChat=function(){if(this.isPersistentChat()&&!this.isPersistentChatActive()){this.removeChatInstance();}};GR34.prototype.setDisconnectFlag=function(value,reason){var disconnectVar=PM.getVar("disconnect");if(!disconnectVar){disconnectVar=new VER34("disconnect",0,resources["state"],"_dcnt",function(o){return parseInt(o);});disconnectVar.init();disconnectVar.z3(value);PM.addVar(disconnectVar);}else{disconnectVar.z3(value);}
...
function reinitializeChatWithNewId(){CHM.reinitializeChatWithNewId();}
...
win.Inq.CMSDK=FP.getCMSDKInst();win.Inq.ciSendText=sendText;win.Inq.isInqInitialized=isInitialized;win.Inq.API.get=APIGetter;}
...
function setPersistCustID(persistCustID){var persistDataVar=new VER34("pcID",{},resources["session"]);var pcID=persistDataVar.z0();if(!objectEquals(pcID,persistCustID)&&!CHM.getChat()){ROM.post(urls.getCustIDURL,{"siteID":Inq.getSiteID()},null,null,setCustomerId);}
persistentCustomerID=persistCustID;Inq.save();}
...
var persistDataVar=new VER34("pcID",{},resources["session"]);var persistCustID=persistDataVar.z0();if(persistCustID){setPersistCustID(persistCustID);}
...
var persistCustID=getPersistCustID();if(persistCustID){persistDataVar.z3(persistCustID);}
